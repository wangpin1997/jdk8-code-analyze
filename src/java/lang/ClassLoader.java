...
public abstract class ClassLoader {
    ...
    private static class ParallelLoaders {
         ...
         * 返回{@code true}已成功注册; {@code false} if
         * loader的超类没有注册。
         */
    ...
    protected Class<?> loadClass(String name, boolean resolve)
        ...
        //保证线程安全，枚举为何单例安全，跟这里的同步有关系
        //先javac一个类，然后javap 反编译，你会发现，枚举类成为了final类，他里边的属性都是stataic的
        //由于属性是static的，所以会被类加载然后初始化，加载到初始化的过程都是安全的，为啥，就是因为此方法synchronized关键字
        synchronized (getClassLoadingLock(name)) {
            ...
            if (c == null) {
                ...
                try {
                    if (parent != null) {
                        //委托给父类加载器，并不单单是指ExtClassLoader，因为有可能自定义的类加载器，这里就是个递归，委派到最上层
                        c = parent.loadClass(name, false);
                    } else {
                        //不存在父类加载器，就委托给BootstrapClassLoader，他是一个native方法，由c++编写，java拿不到，所以返回为null
