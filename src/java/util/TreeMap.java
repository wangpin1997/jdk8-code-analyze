 ...
 * @since 1.2
 */
//一个有序key-value的map集合(根据key值大小排序)，底层采用红黑树
//是一个key-value结构的map继承至AbstractMap
//实现了NavigableMap接口，意味着它支持一系列的导航方法。比如返回有序的key集合。因为NavigableMap继承于SortMap
    ...
    implements NavigableMap<K,V>, Cloneable, java.io.Serializable
{
    ...
    //transient关键字的作用是比如你序列化，但是想某个属性不被序列化，就可用此字段修饰
     ...
     */
    //构造一个指定map的TreeMap,即将另外一个treeMap合并，同样比较器comparator为空，使用key的自然顺序排序
     ...
     * @throws NullPointerException if the specified map is null
     */
    //将一个SortedMap合并为构造 ，并根据sortedMap比较器排序
    ...
    public V put(K key, V value) {
        ...
        //记录比较的结果
        //这个比较方法重点说下，当前者大于后者返回1，小于后者返回-1，等于返回0
        int cmp;
        Entry<K,V> parent;
        ...
        if (cpr != null) {
            //do{}while循环，查找key要插入的位置，即找到父节点是谁
            do {
                // 记录上次循环的节点t
                parent = t;
        ...
        //找到父节点后创建新的entry，即存进去当前数据
        Entry<K,V> e = new Entry<>(key, value, parent);
        //新节点的key小于父节点的key，就在左边
        if (cmp < 0)
            parent.left = e;
        //新节点的key大于父节点的key在右边
        else
            parent.right = e;
        //添加元素后，对红黑树进行调整，为啥，因为红黑树是二叉树，叶子节点最大相差层数不能超过1
        fixAfterInsertion(e);
        //数量+1
        size++;
        //fast-fail数量+1
    ...
    //调整红黑树方法，为什么调整，因为红黑树是近似平衡二叉树，叶子节点度相差不超过较小节点的1倍
    //当咱们新添加后可能会导致度差超过1
    /** From CLR */
