...
//     }
     ...
     */
    //无界队列，并不是无界，只是最大容量是Integer的最大值,基本算无界了
    ...
    public boolean offer(E e) {
        ...
        if (e == null) throw new NullPointerException();
        final AtomicInteger count = this.count;
        //如果当前队列已满，则丢弃将要放入的元素，然后返回false
        if (count.get() == capacity)
            return false;
        int c = -1;
        //不为空也没满，构造新节点，获取putLock锁
        Node<E> node = new Node<E>(e);
        final ReentrantLock putLock = this.putLock;
        putLock.lock();
        try {
        ...
        //说明队列中至少有一个元素，则唤醒notEmpty条件队列中调用了await的线程
        if (c == 0)
            signalNotEmpty();
        return c >= 0;
    }
    ...
    public E poll() {
        ...
        try {
            ...
            if (count.get() > 0) {
                x = dequeue();
                c = count.getAndDecrement();
                //c>1,说明队列中至少还有一个空位，唤醒notEmpty条件队列中的await线程
                if (c > 1)
                    notEmpty.signal();
            }
        } finally {
    ...
    public E peek() {
        ...
        takeLock.lock();
        try {
            //获取哨兵的next节点（真正的头节点），为空则返回null，否支返回其值
    ...
    void unlink(Node<E> p, Node<E> trail) {
        ...
        p.item = null;
        //trail的next指向p的next,p就被架空了
        trail.next = p.next;
        if (last == p)
            last = trail;
        //如果当前队列满了，删除后腾出一个空位，唤醒等待的线程
