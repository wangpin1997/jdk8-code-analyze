...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     */
    //扩容因子，当到达16*0.75=12的时候再添加数据就会触发扩容，
     ...
     */
    //再hash,拿低16位再跟hash码进行hash，避免或减少hash冲突
     ...
     * 返回给定目标容量的两个大小的幂。
     */
    //将大小调整为2的整数次幂，如cap=15则返回16，cap=17则返回32
     ...
     * true (relayed to method afterNodeInsertion).只有构造的时候才为false
     */
    //将一个map添加到当前map中
    final void putMapEntries(Map<? extends K, ? extends V> m, boolean evict) {
        ...
        if (s > 0) {
            ...
            if (table == null) { // pre-size
                ...
                //当t大于下一步要扩容的大小时
                if (t > threshold)
                    //对map的大小调整为2的整数次幂，如果小于则不需要调，因为本来就是规则的，初始化就调好啦
                    threshold = tableSizeFor(t);
            }
            //如果参数map的size比扩容阈值大，就扩容
            else if (s > threshold)
                resize();
            //遍历添加到map中
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            if (p.hash == hash &&
                ...
                // i.  遍历table[i]，判断Key是否已存在：采用equals（） 对比当前遍历节点的key 与 需插入数据的      key：
     ...
     */
    //重置大小，即扩容方法
    //调用的两种情况，初始化的时候，内存小扩容的时候
    final Node<K,V>[] resize() {
        //接收之前的map
        Node<K,V>[] oldTab = table;
        //为空则之前的长度是0，否则取其length
     ...
     * linked node.
     */
    //红黑树节点实现类，继承自LinkedHashMap.Entry ，它又继承于HashMap.Node
