    ...
    implements java.io.Serializable, Cloneable
{
     ...
     */
    //key值集合，也就是充当缓存咯，transient关键字代表不被序列化
     ...
     */
    //构建一个EnumMap对象,
    //EnumMap enumMap=new EnumMap<>(? extends Enum)
    //如Person是一个枚举类EnumMap enumMap=new EnumMap<>(Person.class)
     ...
     */
    //带有枚举的构建，相当于就是把传进来的EnumMap复制到当前
    ...
    public EnumMap(Map<K, ? extends V> m) {
        ...
        if (m instanceof EnumMap) {
            ...
            //如果不是，切map的size为0，抛出异常
            if (m.isEmpty())
                throw new IllegalArgumentException("Specified map is empty");
            //取出其中的属性，putAll创建一个EnumMap
     ...
     *            key
     */
    //判断是否包含某个key,先验证合法，在验证key对应的值是否为null,key.ordinal()方法是返回该属性在枚举类中的序列化
